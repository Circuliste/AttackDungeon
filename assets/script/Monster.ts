...
export default class NewClass extends cc.Component {
    ...
    changeLife(amount:number){
        this._life += amount
        if(this._life<=0){
            let event = new cc.Event.EventCustom('monsterDestroy',true)
    ...
    findTarget(){
        this._target = Common.findNearestNode(Common.soldierNodes,this.node)
    }
    ...
    fire(){
        ...
        if(!(this._target===this.node)){
            if(Common.isInRange(this.node,this._target,this._range)){
                this._target.getComponent("Soldier").changeLife(-this._damage)
            }
        }
    }
    ...
    onDestroy(){
        Common.deleteItemFromArray(Common.monsterNodes,this.node)
    }
